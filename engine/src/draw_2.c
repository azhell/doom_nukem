...
void			drawline(t_main *m, t_vertex s, t_vertex e)
{
	...
	line.delta_x = fabs(e.x - s.x);
	line.delta_y = fabs(e.y - s.y);
	line.sign_x = s.x < e.x ? 1 : -1;
	line.sign_y = s.y < e.y ? 1 : -1;
	line.error = line.delta_x - line.delta_y;
	ft_put_pixel(m, e.x, e.y, 0xff00ff);
	while (s.x != e.x || s.y != e.y)
	{
		ft_put_pixel(m, s.x, s.y, 0xff00ff);
